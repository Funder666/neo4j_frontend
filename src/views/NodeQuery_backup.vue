            ...
            <div class="panel-content">
              ...
              <div v-if="isAdmin" class="panel-actions">
                ...
                <el-button 
                  type="info" 
                  class="action-btn"
                  @click="viewRelationships(selectedNode)"
                >
...
const searchNodes = async () => {
  ...
  try {
    ...
    // 使用后端API搜索节点
    const response = await apiService.searchNodes(queryText.value, selectedLabel.value, limit.value)
    
    results.value = response.nodes.map(item => ({
      id: item.n.id,
      labels: item.n.labels,
...
const viewRelationships = async (node) => {
  ...
  try {
    ...
    relationships.value = response.relationships.map(record => {
      const startNode = record.n
      const endNode = record.m
      const relation = record.r
      
      const isOutgoing = startNode.id === node.id
